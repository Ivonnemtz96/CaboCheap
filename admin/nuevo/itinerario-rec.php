...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
    ...
    if ($descripcion==""){
		...
		$tariCount	=	$db->getQueryCount('itinerarioRec','id');
		if($tariCount[0]['total']<1000){
			...
			$insert	=	$db->insert('itinerarioRec',$data);
            
			if($insert){
                
                    //SUMAMOS +1 A LAS PROPIEDADES DE ESTE USUARIO
                    $Sum = (($itinerario['recCount'])+1);
				...
				header('location:/admin/nuevo/itinerario-rec?itiId='.($itinerario['id']).'');//exito
				exit;
			}else{
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                      <h4>Agrega itinerarios para el viaje de: <?php echo ($itinerario['cliente'])?></h4>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <?php
                                            if (($itinerario['dias'])>0){
                                            
                                                for ($i = 0; $i < $itinerario['dias']; $i++) {
                                                ?>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $Data = $db->getAllRecords('itinerarioRec','*','AND itinerario='.$itinerario['id'].' ORDER BY dia ASC LIMIT '.$itinerario['recCount'].'');
                                  if (count($Data)>0){
                                      $y	=	'';
                                          foreach($Data as $item){
                                              ...
                                              <tr>
                                                    ...
                                                    <td class="text-rigth">
                                                        <a href="/admin/editar/itinerario-rec?editId=<?php echo $item['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                        <a href="/admin/borrar/itinerario-rec?delId=<?php echo $item['id']; ?>" onClick="return confirm('¿Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                    </td>
                                              </tr>
