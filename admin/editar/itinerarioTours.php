...
require_once($_SERVER["DOCUMENT_ROOT"]."/include/funciones.php");
    ...
    if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
        $itinerarioSel  =  $db->getAllRecords('itinerariosTours','*',' AND id="'.$_REQUEST['editId'].'"LIMIT 1');
    }
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
    ...
    if ($descripcion==""){
        ...
        $update	=	$db->update('itinerariosTours',$data,array('id'=>($_REQUEST['editId'])));

        if($update){
            setcookie("msg","tariup",time() + 2, "/");
            header('location:/admin/nuevo/itinerarioTours?itiId='.($itinerarioSel['itinerario']).'');//exito
            exit;}
        else{
            setcookie("msg","ups",time() + 2, "/");
            header('location:/admin/nuevo/itinerarioTours?itiId='.($itinerarioSel['itinerario']).''); //sin cambios
            exit;
        }
    }
}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Nombre</label>
                                        <input name="nombre" class="form-control" type="text" value="<?php echo $itinerarioSel['nombre']; ?>" required>
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="descripcion" class="form-control" cols="5" rows="2" required><?php echo $itinerarioSel['descripcion']; ?></textarea>
                                        <div class="invalid-feedback">
