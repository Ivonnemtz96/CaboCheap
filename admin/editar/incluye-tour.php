...
if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	...
	$incluyeSel  =  $db->getAllRecords('toursIncluye','*',' AND id="'.$_REQUEST['editId'].'"','LIMIT 1');
    
        if (empty($incluyeSel)) { //SI NO EXISTE ES QUE NO HAY UN ID VALIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
            header('location:/admin/nuevo/incluye-tour');
            exit;
        } else {$incluyeSel  =  $incluyeSel [0];}
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($descripcion==""){
        ...
        if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	       ...
	       $update	=	$db->update('toursIncluye',$data,array('id'=>($_REQUEST['editId'])));
    
            if($update){
                setcookie("msg","carup",time() + 2, "/");
                header('location:/admin/nuevo/incluye-tour'); //Exito en el cmabio
                exit;}
                else{
            ...
            <div class="col-12 col-md-6 col-lg-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>Editar una característica (Incluye en tour)</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input name="descripcion" value="<?php echo ($incluyeSel['descripcion']);?>" type="text" class="form-control">
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-2">
