...
require_once($_SERVER["DOCUMENT_ROOT"]."/include/funciones.php");
    ...
    if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
        $itinerarioSel  =  $db->getAllRecords('toursItinerario','*',' AND id="'.$_REQUEST['editId'].'"LIMIT 1');
    }
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
    ...
    if ($descripcion==""){
        ...
        if($update){
            ...
            header('location:/admin/nuevo/itinerario-tour?tourId='.($itinerarioSel['tour']).'');//exito
            exit;}
        else{
            setcookie("msg","ups",time() + 2, "/");
            header('location:/admin/nuevo/itinerario-tour?tourId='.($itinerarioSel['tour']).''); //sin cambios
            exit;
        }
    }
}
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <a href="/admin/nuevo/itinerario-tour?tourId=<?php echo $itinerarioSel['tour']; ?>" type="button" class="btn btn-icon icon-left btn-success">
